#!/usr/bin/env ts-node

import 'dotenv/config';
import { v4 as uuidv4 } from 'uuid';
import bcrypt from 'bcrypt';
import { query } from '../src/core/db';

async function main() {
  const username = process.env.FIRST_ADMIN_USERNAME || process.argv[2];
  const email = process.env.FIRST_ADMIN_EMAIL || process.argv[3] || null;
  const password = process.env.FIRST_ADMIN_PASSWORD || process.argv[4];

  if (!username || !password) {
    console.error('Uso: npm run create-admin -- <username> <email?> <password>  OR set FIRST_ADMIN_USERNAME, FIRST_ADMIN_PASSWORD en .env');
    process.exit(1);
  }

  // check existing
  const existing = await query('SELECT id FROM auth WHERE username = ? LIMIT 1', [username]);
  if (existing.length > 0) {
    console.error('Ya existe un usuario con ese username. Abortando.');
    process.exit(1);
  }

  const id = uuidv4();
  const passwordHash = await bcrypt.hash(password, 10);

  await query(
    'INSERT INTO auth (id, username, email, password_hash, is_admin, created_at, updated_at) VALUES (?, ?, ?, ?, 1, NOW(), NOW())',
    [id, username, email, passwordHash]
  );

  console.log('Admin creado con éxito: ', { id, username, email });
  process.exit(0);
}

main().catch((err) => {
  console.error('Error al crear admin:', err && err.message ? err.message : err);
  process.exit(1);
});